...
int main(int argc, char** argv)
{
    ...
    if(std::regex_search(info, match, regex))
    {
        std::cout << "Searching for first ocurrence: " << std::endl 
    ...
    for (auto it = std::sregex_iterator{
        std::begin(info), std::end(info), regex}; it != end; ++it)
    {
        std::cout << '\'' << (*it)[1] << "'='"
                  << (*it)[2] << '\'' << std::endl;
    }
    ...
    for (auto it = std::sregex_token_iterator{
        std::begin(info), std::end(info), regex }; it != end_tok; ++it)
    {
